...
from emotionClassifier import emotion_testing
...
photo = PhotoImage(file="Artist_bg.jpg")
...
name_label = tk.Label(root, text='Enter Name of Singer',
                      font=('calibre',
                            10, 'bold'))
...
danceSongs = cluster.apply(lambda x: x.sort_values(["danceability"], ascending=False))
danceSongs.reset_index(level=0, inplace=True)
...
def get_results(emotion_code):
    ...
    if emotion_code == 0:
        if hand != "none":
            happy_set.append(danceSongs[danceSongs['kmeans'] == 0]['name'].head(NUM_RECOMMEND))
        else:
...
emote = (emotion_testing())
hand = emote[1]
emotion_word = emote[0]
if emotion_word == 'sad':
    emotion_code = 1
else:
    emotion_code = 0
...
message = tk.Label(
    ...
    bg="black", fg="#3DFD83", width=50,
    height=3, font=('times', 32, 'bold'))

message.place(x=200, y=20)
pred = tk.Button(window, text="print",
                 command=final, fg="black", bg="#878787",
                 width=20, height=3, activebackground="#3DFD83",
                 font=('times', 15, ' bold '))
pred.place(x=200, y=500)

quitWindow = tk.Button(window, text="Quit",
                       command=window.destroy, fg="black", bg="#878787",
                       width=20, height=3, activebackground="#3DFD83",
                       font=('times', 15, ' bold '))
quitWindow.place(x=1100, y=500)

image1 = Image.open("spotify.jpeg")
new_image = image1.resize((600, 500))
